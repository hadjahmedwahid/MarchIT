...
<div >
	 ...
	 <?php 
	  include 'ConnexionDB.php';
	 	$f= $bdd ->query('SELECT * FROM `facture` order by idfact desc');
		$cash1 = "<ul>" ;
		  while($data2 = $f->fetch()){
			$cash1 .= "<li><hr>"; 
			$cash1 .= '<form action="fact.php" method="POST">';
			$cash1 .= '<input class="form-control" id="exampleInputName2" value="';
			$cash1 .=    $data2["idfact"].'" name="f" type="hidden">';
			$cash1 .= $data2["idfact"];
			$cash1 .= "&nbsp;&nbsp;";
			$cash1 .= $data2["datef"];
			$cash1 .= "&nbsp;&nbsp;";
			$cash1 .= $data2["nomclient"];
		    $cash1 .='<div align="center"><input value="BON" type="submit"></div>';
